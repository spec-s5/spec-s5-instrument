...
# pm_err is NaN for z >= 21 because the default bp_rp=1.2 converts those to G > 20.7.
EXPECTED = {
    ...
    21: dict(vrad_err=2.694952, pm_err=None,      dist_err_frac=0.554783, snr_median_zarm=12.338348),
    22: dict(vrad_err=6.477136, pm_err=None,      dist_err_frac=0.623753, snr_median_zarm=4.911985),
    23: dict(vrad_err=15.842486, pm_err=None,     dist_err_frac=0.631276, snr_median_zarm=1.955496),
}
...
def test_pm_err(m):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        r = compute_measurement_errors(m)
    expected = EXPECTED[m]["pm_err"]
    if expected is None:
        assert math.isnan(r["pm_err"])
    else:
        assert r["pm_err"] == pytest.approx(expected, rel=1e-4)
...
# When the input is Gaia G, the Gaia DR5 PM lookup uses G directly.
# G > 20.7 returns NaN.
...
def test_pm_err_gaia_g_input(m):
    ...
    expected = EXPECTED_PM_ERR_GAIA_G[m]
    if expected is None:
        assert math.isnan(r["pm_err"])
    else:
        assert r["pm_err"] == pytest.approx(expected, rel=1e-4)
